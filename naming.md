...
Мы строим имена транспортных моделей следующим образом:

`БизнесСоставляющаяПринадлежностьКСервисуУточнинениеВерсия`

Примеры для репозиторных сущностей:
`UserTable`, `OrderRecord`.

Примеры для моделей API:
`UserRequestV3`, `OrderResponseV7`.
Для моделей, не имеющих выраженного направления движения или движимого в обе стороны (в запросе и ответе, как, например, enum), используем окончание `Dto`:
`OrderStatusDto`. `ContactTypeDto`.
